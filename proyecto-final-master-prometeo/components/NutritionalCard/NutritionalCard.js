...
export const printNutritionalInfo = (
  result,
  infoQuantity,
  nameUnit,
  infoFood
...
<h5>Tamaño de la Porción </h5><span class="spanInfoNutritional">${
    infoQuantity || "N/A"
  } ${nameUnit || "N/A"}</span>
</div>
<div class="nutrientContainer border-bottom">
<h5 class="h4-green">Energía:</h5><span class="spanInfoNutritional"> ${
    result.ENERC_KCAL?.quantity?.toFixed(0) || "N/A"
  } ${result.ENERC_KCAL?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer border-bottom">
<h5 class="h4-green">Proteína:</h5><span class="spanInfoNutritional"> ${
    result.PROCNT?.quantity?.toFixed(2) || "N/A"
  } ${result.PROCNT?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer">
<h5 class="h4-green">Carbohidratos:</h5><span class="spanInfoNutritional"> ${
    result.CHOCDF?.quantity?.toFixed(2) || "N/A"
  } ${result.CHOCDF?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer">
<h5 class="h4-green">Fibra:</h5><span class="spanInfoNutritional"> ${
    result.FIBTG?.quantity?.toFixed(2) || "N/A"
  } ${result.FIBTG?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer border-bottom">
<h5 class="h4-green">Azúcar:</h5><span class="spanInfoNutritional"> ${
    result.SUGAR?.quantity?.toFixed(2) || "N/A"
  } ${result.SUGAR?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer">
<h5 class="h4-green">Grasa:</h5><span class="spanInfoNutritional"> ${
    result.FAT?.quantity?.toFixed(2) || "N/A"
  } ${result.FAT?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer">
<h5 class="h4-green">Grasas Saturadas:</h5><span class="spanInfoNutritional"> ${
    result.FASAT?.quantity?.toFixed(2) || "N/A"
  } ${result.FASAT?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer">
<h5 class="h4-green">Grasas Poliinsaturadas:</h5><span class="spanInfoNutritional"> ${
    result.FAPU?.quantity?.toFixed(2) || "N/A"
  } ${result.FAPU?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer border-bottom">
<h5 class="h4-green">Grasas Monoinsaturadas:</h5><span class="spanInfoNutritional"> ${
    result.FAMS?.quantity?.toFixed(2) || "N/A"
  } ${result.FAMS?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer border-bottom">
<h5 class="h4-green">Colesterol:</h5><span class="spanInfoNutritional"> ${
    result.CHOLE?.quantity?.toFixed(2) || "N/A"
  } ${result.CHOLE?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer border-bottom">
<h5 class="h4-green">Sodio:</h5><span class="spanInfoNutritional"> ${
    result.NA?.quantity?.toFixed(2) || "N/A"
  } ${result.NA?.unit || "N/A"}</span>
</div>
<div class="nutrientContainer border-bottom">
<h5 class="h4-green">Potasio:</h5><span class="spanInfoNutritional"> ${
    result.K?.quantity?.toFixed(2) || "N/A"
  } ${result.K?.unit || "N/A"}</span>
</div>
...
export const printIDR_Info = (result) => {
  ...
  if (calculatedCalories > 100) {
    ...
    // Si no sobrepasa el 100%, usar verde y gris
...
export const printBreakdownCalories = (result) => {
  ...
  const percentageValueCHOCDF =
    (result.CHOCDF_KCAL.quantity * 100) / result.TotalCalories; //% que representa ente macro
  const percentageValueFAT =
    (result.FAT_KCAL.quantity * 100) / result.TotalCalories; //% que representa ente macro
  const percentageValuePROCNT =
    (result.PROCNT_KCAL.quantity * 100) / result.TotalCalories; //% que representa ente macro
  divContainerBreaksown.innerHTML = `
    ...
    <ul>
    <li class="h4-green">Carbohidratos (${
      percentageValueCHOCDF?.toFixed(1) || 0
    }%)<span class="square square-carbohydrates"></span></li>
    <li class="h4-green">Grasa (${
      percentageValueFAT?.toFixed(1) || 0
    }%)<span class="square square-fat"></span></li>
    <li class="h4-green">Proteínas (${
      percentageValuePROCNT?.toFixed(1) || 0
    }%)<span class="square square-protein"></span></li>
</ul>
    <div class="containerCanvas">
  ...
  new Chart(chartBreakdown, {
    ...
    data: {
      ...
      datasets: [
        {
          data: [
            percentageValueCHOCDF.toFixed(1),
            percentageValueFAT.toFixed(1),
            percentageValuePROCNT.toFixed(1),
          ],
          backgroundColor: [
            "rgb(10, 85, 47)",
            "rgb(231, 76, 60)",
...
<li class="nutritionalSummaryLi">${
    result.TotalCalories
  } Kcal<span >🔥</span><span > calorias</span></li>
<li class="nutritionalSummaryLi">${
    result.FAT?.quantity?.toFixed(2) || "N/A"
  } g<span >🥑</span><span > grasas</span></li>
<li class="nutritionalSummaryLi">${
    result.CHOCDF?.quantity?.toFixed(2) || "N/A"
  } g<span >🥔</span><span > carbohidratos</span></li>
<li class="nutritionalSummaryLi">${
    result.PROCNT?.quantity?.toFixed(2) || "N/A"
  } g<span >🍗</span><span > proteinas</span></li>
</ul>
`;
...
export const printNutritionalMicronutrients = (result) => {
  ...
  const labelsIDR = data.map((label) => {
    return parseFloat(label) > 100
      ? " % Exdece los valores diarios recomendados"
      : "% IDR";
  });
